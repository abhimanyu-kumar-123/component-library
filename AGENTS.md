...
```
- `variant`: `primary` (indigo gradient, teal inner stroke) · `secondary` (glass white, dark text) · `ghost` (transparent, brand-blue text, link-style) · `destructive` (solid `danger` red, white text/icon — mirrors primary) · `destructive-secondary` (white glass, red text + red icon — mirrors secondary) · `on-dark` (solid white, dark text) · `on-dark-secondary` (translucent outline, white text). Destructive variants support every `size` + with/without icon, like the others. Figma node `4803:8461`.
- **Rule — `on-dark` / `on-dark-secondary` are for DARK / coloured surfaces ONLY** (e.g. the blue `ActionDeck` / `Card`): `on-dark` is the primary (solid white), `on-dark-secondary` the secondary (translucent outline), both with a white focus ring. Never use them on light backgrounds — use `primary` / `secondary` there.
...
- **`Toggle`** (`@/components/ui/toggle`) — **iOS-style on/off** toggle: a pill track with a sliding white knob (`bg-gradient-primary` track when on, `border-divider` grey when off). Use for a **single binary setting** (e.g. Cash-on-delivery, notifications). **Rules:** use `Switch` for 2–3 *view* options and `Checkbox` for multi-select — `Toggle` is specifically the on/off affordance. Controlled (`checked`) / uncontrolled (`defaultChecked`), optional `label` (right of the toggle), `disabled`.
  `<Toggle label="Accept Cash on Delivery" checked={cod} onCheckedChange={setCod} />`
...
- **`SectionCard`** (`@/components/ui/section-card`) — a glass card with the section title **inset in its header** (Figma `5304:968`): inline icon + `type-h2` title (+ optional compact "View all" via `onViewAll`) over a body. `surface`: `white` (default — inner white panel, e.g. *Recent conversation*) | `glass` (transparent, for white cards-on-glass, e.g. *Your journey*). Use `SectionCard` when the title belongs **inside** the card; use `SectionTitle` when the header sits **above** separate cards.
  `<SectionCard icon={<ChatTeardropText weight="fill" />} title="Recent conversation" onViewAll={fn}>…</SectionCard>`
- **`ProgressBar`** (`@/components/ui/progress-bar`) — slim progress bar (Figma `4909:4892`): grey `border-divider` track + light→solid gradient fill. `tone`: `orange` (default) | `blue` | `red` | `green`; `value` 0–100.
  `<ProgressBar tone="red" value={86} />`
- **`Stepper`** (`@/components/ui/stepper`) — horizontal progress stepper (Figma `5190:5838`) for a short go-live / onboarding journey. `steps={[{ label, status }]}` with `status`: `done` (filled brand dot) · `current` (brand ring) · `goal` (brand target) · `upcoming` (grey ring). The connector into a node is **solid** when the previous node is `done`, **dotted** otherwise; labels sit under the track. Use inside the go-live card; for a single percentage use `ProgressBar`, for a long task list use a vertical timeline of cards.
  `<Stepper steps={[{label:"12 Jun",status:"done"},{label:"15 Jun",status:"current"},{label:"30 Jun",status:"goal"}]} />`
- **`InfoNote`** (`@/components/ui/info-note`) — a calm, muted inline aside: a grey (`surface-muted`) rounded box with a leading `Info` icon + `text-secondary` `type-body-2` text. Use under a card/section for a passive contextual note (e.g. "This was discussed in the last conversation"). Not for warnings/errors (use a `Pill` / field error) or primary content.
  `<InfoNote>This was discussed in the last conversation</InfoNote>`
- **`DataCard`** (`@/components/ui/data-card`) — data-representation metric surface (Figma `4909:5126`). `type`: `single` (one metric) · `double` (two metrics split by a divider) · `triple` (three centered metrics) · `ratings` (star row). Each `DataMetric` = `{ label, value, cap?, delta?:{value,tone}, progress?:{pct,tone}, icon? }`; `cap` (e.g. "/ ₹5.0 L") shows when there's no `delta`. `delta` = green/red pill; `progress` uses `ProgressBar` (tones `orange` | `blue` | `red` | `green`). Ratings: `rating`, `outOf`, `ratingLabel`, `ratingNote`.
  `<DataCard type="double" metrics={[{label,value,progress:{pct:48}},{label,value,delta:{value:"15%"}}]} />`
- **`ArtifactCard`** — a uniform card for the **Collections** panel (`/artifacts`). Every card is the same structure (fixed-aspect type-specific CSS preview thumbnail — `report`/`document` → spreadsheet mock · `website`/`catalog`/`brand-kit` → storefront mock — + `name` + `time`), so the layout never shifts. **`completed`** cards are clean/passive (no extra UI). **`ongoing`** (blue `brand-primary` + DotsThreeCircle) and **`action-needed`** (orange `warning-default` + Lightning) cards overlay a solid status pill on the thumbnail (so height stays identical). Type previews: `report` → spreadsheet · `document` → text lines · `website`/`catalog`/`brand-kit` → storefront. Tapping (`onClick`) opens the **preview sheet** (`BottomSheet`: Share + Download `RoundButton` + Continue). The Collections screen has **no filter pills** — calm, scannable, completed-by-default. Entry point: the `SquaresFour` icon in the chat header → `/artifacts`.
...
- **`CallbackBar`** (`@/components/ui/callback-bar`) — a "Call <POC>" help bar (Figma `5284:1003`): avatar + title + subtitle + green `Phone` button. **Layout rule (always): full screen width, floating directly above `BottomNav` with NO gap** — so it has rounded *top* corners only, a flat bottom, and an *upward* shadow (`0 -10px 30px`). Render it as the first child of the fixed bottom region, immediately before `BottomNav` (no margin between them).
  `<CallbackBar src={url} title="Call Anirudh" description="…" onCall={fn} />`
...
- **`ReportChip`** (`@/components/ui/report-chip`) — inline report/file attachment rendered as the children of an AI `ChatBubble` (Figma `4929:11765`): a white XLS-icon box + filename (`report.xls`). The AI posts it when a report-generation request ("Generate report for last 3 months", "Create quarterly sales report", "Prepare analytics summary") completes — see `src/app/chat/page.tsx` (`isReportRequest` / `generateReport`). On completion: (1) registers a `report` artifact via `addGeneratedArtifact` in **`@/lib/artifacts-store`** (localStorage + change event) so it shows as the newest card on `/artifacts`; (2) glows the header **Collections** icon for 15s with the soft red breathing `animate-artifact-glow` (icon fills + turns `danger`, then eases back to black). Tapping the chip — or the report card in Collections — opens the report preview `BottomSheet`.
  `<ChatBubble variant="ai"><ReportChip name="report.xls" onClick={openSheet} /></ChatBubble>`
